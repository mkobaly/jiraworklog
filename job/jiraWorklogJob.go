...
type JiraDownloadWorklogs struct {
	cfg    *jiraworklog.Config
	writer writers.Writer
	logger *log.Entry
}

func NewJiraDownloadWorklogs(cfg *jiraworklog.Config, writer writers.Writer, logger *log.Entry) *JiraDownloadWorklogs {
...
func (j *JiraDownloadWorklogs) GetName() string {
	return "JiraDownloadWorklogs"
}

func (j *JiraDownloadWorklogs) GetInterval() time.Duration {
	return time.Second * 90
}

func (j *JiraDownloadWorklogs) Run() error {
	...
	for _, wd := range details {
		...
		issue, err := jira.Issue(wd.IssueID)
		if err != nil {
			switch err {
		...
		if issue.HasParent() {
			issueParent, err = jira.Issue(issue.ParentID())
		}
...
func (j *JiraDownloadWorklogs) okToProcess(w jiraworklog.Worklog, userNames []string) bool {
	if len(userNames) == 0 {
		return true
	}
...
func (j *JiraDownloadWorklogs) convert(w jiraworklog.Worklog, i jiraworklog.Issue, parentIssue jiraworklog.Issue) types.WorklogItem {
